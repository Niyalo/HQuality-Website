...
import type { SanityImageAssetDocument, SanityAssetDocument } from 'next-sanity';
...
export default function AddClientModal({ isOpen, onClose, onSuccess, clientToEdit }: AddClientModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        const fetchAgents = async () => {
          const query = `*[_type == "user" && role == "agent"]{_id, firstname, lastname}`;
          setAvailableAgents(await client.fetch<Agent[]>(query));
        };
        fetchAgents();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let newContractAssets: SanityAssetDocument[] = [];
      if (newContracts.length > 0) {
        newContractAssets = await Promise.all(
          newContracts.map(c => client.assets.upload('file', c.file))
        );
      }
      ...
      const finalContracts = [
        ...
        ...newContracts.map((c, i) => ({
          _key: newContractAssets[i]._id,
          title: c.title,
          file: { asset: { _ref: newContractAssets[i]._id } }
        }))
      ];
      
      const payload = {
        first_name: firstName, last_name: lastName, email, address,
        contact, agentId: selectedAgentId || null,
      ...
      const response = await fetch(isEditMode ? '/api/edit-client' : '/api/add-client', {
        ...
        body: JSON.stringify({ ...payload, clientId: isEditMode ? clientToEdit?._id : undefined }),
      });
